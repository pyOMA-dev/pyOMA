...
The figures below are generated from the bundled steel-frame example data (see
:doc:`example_data`).  The code that produces each figure is shown in the
relevant step below and is also included in the example Jupyter notebooks.

.. figure:: _static/doc_geometry.png
   :width: 380
   :alt: 3-D geometry with nodes, structural lines, and channel-DOF arrows

   **Geometry view** — nodes (grey dots), structural lines, and channel-DOF
   arrows showing the oblique measurement directions of each laser head for
   setup 1.  Generated by :class:`~pyOMA.core.PlotMSH.ModeShapePlot`.

.. figure:: _static/doc_signals.png
   :width: 700
   :alt: Raw velocity time series (left) and power spectral density (right)

   **Measurement signals** — velocity time series (left column) and power
   spectral densities (right column) for the three vibrometer channels of
   setup 1 after decimation to ≈ 28 Hz.

.. figure:: _static/doc_stabilization.png
   :width: 700
   :alt: Stabilization diagram with automatically selected poles highlighted

   **Stabilization diagram** — all poles computed by SSI-cov/ref up to model
   order 40 (grey: unstable, black: stable).  Red diamonds mark the
   automatically selected physical modes.

.. figure:: _static/doc_modeshapes.png
   :width: 700
   :alt: First four mode shapes of the steel-frame test structure side-by-side

   **Mode shapes 1–4** (PoGER, all 15 setups merged) — first bending in *x*,
   first bending in *y*, torsion, and second bending, plotted with exaggerated
...
To visualise the geometry together with sensor positions before running
identification, use :class:`~pyOMA.core.PlotMSH.ModeShapePlot` with only
geometry and ``prep_signals`` supplied:

.. code-block:: python

   from pyOMA.core import ModeShapePlot

   geo_plot = ModeShapePlot(geometry_data=geometry_data, prep_signals=prep_signals)
   geo_plot.draw_nodes()
   geo_plot.draw_lines()
   geo_plot.draw_chan_dofs()   # arrows showing sensor measurement directions
...
To inspect signals and correlation functions before running identification, use
:class:`~pyOMA.core.PreProcessingTools.SignalPlot`:

.. code-block:: python

   from pyOMA.core import SignalPlot

   sig_plot = SignalPlot(prep_signals)

   # Time histories and power spectral densities, one row per channel
   sig_plot.plot_signals(channels=[0, 1, 2], per_channel_axes=True, psd_scale='db')

   # Correlation functions (pass timescale='lags' after calling prep_signals.correlation())
   sig_plot.plot_signals(channels=[0, 1, 2], per_channel_axes=True, timescale='lags')
...
The bundled example data originates from ambient vibration measurements on a
steel-frame test structure using a Polytec PSV 400-3D three-head scanning laser
vibrometer, with each laser head measuring in an oblique (skewed) direction
relative to the structure.  A detailed description of the test structure, the
3D-SLDV measurement principle, and the 15-setup scan arrangement is given on the
:doc:`example_data` page.
